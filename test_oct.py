...
def evaluate_test_set(model, test_dir, transform, class_to_idx, device):
    """Evaluate model on a test set with class subdirectories (including nested subfolders)"""
    all_predictions = []
    all_labels = []
    all_confidences = []
    all_file_paths = []
    ...
    for class_name in os.listdir(test_dir):
        ...
        # Recursively walk through all subdirectories within the class directory
        image_count = 0
...
def calculate_sensitivity_specificity(cm, class_names):
...
def plot_sensitivity_specificity(sensitivity, specificity, class_names):
    """Plot sensitivity and specificity for each class"""
    x = np.arange(len(class_names))
    width = 0.35
    
    fig, ax = plt.subplots(figsize=(12, 6))
    bars1 = ax.bar(x - width/2, sensitivity, width, label='Sensitivity', alpha=0.8)
    bars2 = ax.bar(x + width/2, specificity, width, label='Specificity', alpha=0.8)
...
def main():
    ...
    print("Searching for images in class directories and all subdirectories...")
    ...
    sensitivity, specificity = calculate_sensitivity_specificity(cm, class_names)
    ...
    print("\nPer-Class Metrics:")
    print(f"{'Class':<20} {'Sensitivity':<12} {'Specificity':<12}")
    print("-" * 44)
    for i, class_name in enumerate(class_names):
        print(f"{class_name:<20} {sensitivity[i]:<12.4f} {specificity[i]:<12.4f}")
    ...
    plot_sensitivity_specificity(sensitivity, specificity, class_names)
    print("Sensitivity and specificity chart saved to sensitivity_specificity.png")
